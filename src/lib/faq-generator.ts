...
export function generateModelFaq({
  ...
  const L = (cs: string, sk: string, uk: string): string => (isSk ? sk : isUk ? uk : cs);
  const fmtNumber = (n: number): string =>
    n.toLocaleString(isSk ? 'sk-SK' : isUk ? 'uk-UA' : 'cs-CZ');
  ...
  const yearPhrase = (): string | null => {
    ...
    return L(`od roku ${model.year_from}`, `od roku ${model.year_from}`, `з ${model.year_from} року`);
  };
  ...
  if (model.power_hp) {
    ...
    const enginePart = model.engine
      ? L(` Pohání ho ${model.engine}.`, ` Poháňa ho ${model.engine}.`, ` Його приводить у рух ${model.engine}.`)
      : '';
    items.push({
      q: L(`Jaký výkon má ${fullName}?`, `Aký výkon má ${fullName}?`, `Яку потужність має ${fullName}?`),
      a: L(
  ...
  if (model.weight_kg) {
    items.push({
      q: L(`Kolik váží ${fullName}?`, `Koľko váži ${fullName}?`, `Скільки важить ${fullName}?`),
      a: L(
  ...
  if (yr) {
    if (model.year_to) {
      items.push({
        q: L(`V jakých letech se ${fullName} vyráběl?`, `V akých rokoch sa ${fullName} vyrábal?`, `У які роки випускався ${fullName}?`),
        a: L(
      ...
      items.push({
        q: L(`Vyrábí se ${fullName} stále?`, `Vyrába sa ${fullName} stále?`, `Чи ${fullName} досі випускається?`),
        a: L(
  ...
  if (model.transmission && VEHICLE_CATEGORIES.has(category)) {
    items.push({
      q: L(`Jakou převodovku má ${fullName}?`, `Akú prevodovku má ${fullName}?`, `Яку коробку передач має ${fullName}?`),
      a: L(
  ...
  if (category === 'kombajny' && model.cutting_width_m) {
    items.push({
      q: L(`Jaký záběr žacího stolu má ${fullName}?`, `Aký záber žacieho stola má ${fullName}?`, `Яку ширину захвату жниварки має ${fullName}?`),
      a: L(
  ...
  if (category === 'kombajny' && model.grain_tank_l) {
    items.push({
      q: L(`Jak velký je zásobník zrna u ${fullName}?`, `Aký veľký je zásobník zrna pri ${fullName}?`, `Який об'єм бункера для зерна у ${fullName}?`),
      a: L(
  ...
  if (model.pracovni_zaber_m) {
    items.push({
      q: L(`Jaký pracovní záběr má ${fullName}?`, `Aký pracovný záber má ${fullName}?`, `Яку робочу ширину захвату має ${fullName}?`),
      a: L(
  ...
  if (model.prikon_traktor_hp_min || model.prikon_traktor_hp_max) {
    ...
    items.push({
      q: L(`Jaký traktor potřebuje ${fullName}?`, `Aký traktor potrebuje ${fullName}?`, `Який трактор потрібен для ${fullName}?`),
      a: L(
  ...
  if (model.typ_zavesu) {
    ...
    const labels = isSk ? labelsSk : isUk ? labelsUk : labelsCs;
    const label = labels[model.typ_zavesu] ?? model.typ_zavesu;
    items.push({
      q: L(`Jak se ${fullName} připojuje k traktoru?`, `Ako sa ${fullName} pripája k traktoru?`, `Як ${fullName} приєднується до трактора?`),
      a: L(
  ...
  if (bazarCount !== undefined && bazarCount > 0) {
    ...
    items.push({
      q: L(`Lze koupit ${fullName} v bazaru?`, `Dá sa kúpiť ${fullName} v bazári?`, `Чи можна купити ${fullName} на ринку вживаної техніки?`),
      a: L(
