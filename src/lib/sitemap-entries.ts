// Zdroj pravdy pro VŠECHNY sitemapy. Sestaví kompletní seznam URL; routy
// /sitemap.xml (index) a /sitemap-[section].xml si ho jen rozdělí na sekce.
// Dřív to byla jedna routa s ~40 tisíci URL v jednom 8MB souboru — Google to
// sice zvládne, ale v Search Consoli pak nejde poznat, KTERÁ sekce se
// neindexuje. Dělení podle sekcí dělá pokrytí čitelným.
import type { APIRoute } from 'astro';
import { getCollection } from 'astro:content';
import { sitemapSection, type SitemapSection } from './sitemap-sections';
import { getAllBrands, getAllModels, seriesFamily, FUNCTIONAL_GROUPS } from './stroje';
import { getAllDruhy } from './plemena';
import { getAllFarms, regionsWithEnoughFarms } from './farmy';
import { getAllVcely, getAllVybaveni, getAllMed } from './vcelarstvi';
import { getAllHlemyzdi } from './hlemyzdi';
import { listPlodiny, listIndexableOdrudy, listSkupiny, listIndexableUdrzovatele, udrzovatelSlug } from './plodiny';
import statsPlData from '../data/agro-stats-pl.json';
import { listIndexableChoroby } from './choroby';
import { expandedComparisonPairs, implementComparisonPairs } from './comparator';
import { brandPairs } from './brand-comparator';
import { createAnonClient } from './supabase';
import { listPublishedForMaintenance } from './akce-supabase';
import { AKCE_TYP_SLUGS } from './akce-constants';
import { getKraje } from './lokality';
...
import { NEWS_CATEGORIES } from './news-category';
...
export function renderUrl(u: UrlEntry): string {
...
export async function buildSitemapEntries(): Promise<UrlEntry[]> {
  ...
  const staticPaths: Array<[string, UrlEntry['changefreq'], string?, string?]> = [
    ...
    ['/prehled/nejprodavanejsi-kombajny-2025/', 'yearly', '0.8', STATIC_LASTMOD],
    ['/prehled/nejspolehlivejsi-traktory/', 'monthly', '0.8', STATIC_LASTMOD],
  ...
  const { TIER_LISTS } = await import('./tier-lists');
  ...
  const { SLOVNIK } = await import('./slovnik');
  ...
  const { KRAJE } = await import('./cap-dotace');
  ...
  const { CATEGORIES: BAZAR_CATEGORIES, BRANDS: BAZAR_BRANDS } = await import('./bazar-constants');
  ...
  const { landingEntriesForCategory } = await import('./bazar-attribute-landing');
  ...
  const { machines: histMachines, milestones: histMilestones } = await import('./historie');
  ...
  // cs-only technické stránky. Přidané ZA mirrory (stejně jako /historie/),
  // aby se nezrcadlily do sk/uk/pl — obě jsou psané jen česky a mají noHreflang.
...
// re-export, aby routy měly jeden import.
export { SITEMAP_SECTIONS, isSitemapSection } from './sitemap-sections';
export type { SitemapSection } from './sitemap-sections';
export { sitemapSection };

// Routa jedné dílčí sitemapy. Sekce se předává natvrdo ze souboru v pages/,
// protože DYNAMICKÁ routa (/sitemap/[section].xml) by se s `trailingSlash:
// 'always'` dala otevřít jen jako `/sitemap/stroje.xml/` — statické routy
// s příponou Astro z pravidla vyjímá, dynamické ne. Ověřeno na buildu:
// pattern dynamické routy končil `\.xml\/$`, takže URL bez lomítka 404-ovala.
export function sectionSitemapRoute(section: SitemapSection): APIRoute {
  return async () => {
    const urls = (await buildSitemapEntries()).filter((e) => sitemapSection(e.loc) === section);
    // Prázdná sekce = 404, ne prázdný urlset. Prázdnou sitemapu hlásí Search
    // Console jako chybu; index na ni navíc vůbec neodkazuje.
