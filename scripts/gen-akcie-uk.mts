// Vygeneruje ukrajinský overlay sekce /akcie: kurátorské texty firem
// (src/data/akcie-agro-uk.ts) a UI copy (vytiskne blok pro src/i18n/akcie.ts).
//
// Sesterský skript ke gen-akcie-sk.mts. Overlay, ne druhá databáze —
// faktická data (ticker, burza, měna, CEO, rok založení) jsou jazykově
// neutrální a zůstávají v akcie-agro.ts.
//
// Spuštění: npx tsx --env-file=.env scripts/gen-akcie-uk.mts
...
const SYS = `Ти експерт-перекладач з чеської на українську для фінансово-аграрного порталу.

ПРАВИЛА:
- Перекладай ВИКЛЮЧНО українською літературною мовою. Жодного чеського слова.
- НЕ перекладай власні назви: назви компаній, брендів, продуктів, тікери, біржі
  (John Deere, AGCO, Bayer, NYSE, Xetra…), імена людей.
- Числа, валюти, роки, відсотки й одиниці залиш ТОЧНО як є.
- Скорочення розрядів: "mld." → "млрд", "bil." → "трлн".
- Назви країн і міст переклади українською (Německo → Німеччина, Japonsko →
  Японія, Nizozemsko → Нідерланди, Spojené státy → Сполучені Штати).
- Зберігай структуру й кількість елементів у масивах.
...
for (const a of AKCIE as any[]) {
  ...
  for (let pass = 0; pass < 3; pass++) {
    ...
    const fixed = await translate(retry, `firma ${a.ticker}`, ' — попередній переклад не був українською, виправ');
...
const header = `// Ukrajinský overlay kurátorských textů k akciím agro firem (src/data/akcie-agro.ts).
//
...
fs.writeFileSync('src/data/akcie-agro-uk.ts', header + JSON.stringify(out, null, 2).replace(/"([a-z]+)":/g, '$1:') + ';\n', 'utf-8');
console.log(`\n→ src/data/akcie-agro-uk.ts (${Object.keys(out).length} firem)`);

// ── 2) UI copy sekce ─────────────────────────────────────────────────────────
const copy = await translate({ ...content.cs, numberLocale: undefined, bln: undefined }, 'UI texty sekce /akcie');
copy.numberLocale = 'uk-UA';
copy.bln = 'млрд';
fs.writeFileSync('.akcie-copy-uk.json', JSON.stringify(copy, null, 2), 'utf-8');
console.log('→ .akcie-copy-uk.json (blok pro src/i18n/akcie.ts)');
